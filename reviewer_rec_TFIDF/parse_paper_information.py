...
def abc(reviewer_embedding_dict: dict,
    ...
    recall_total = 0
    precision_total = 0
    hit_total = 0  # Variable to track the number of papers with at least one hit
    paper_number = 0
    label_all = []
    pred_all = []
    ...
    for i in range(len(paper_id_list)):
        ...
        if paper_id in review_history_training_dict.keys():
            ...
            # Check if there's at least one match for hit_rate
            if len(set(reviewer_ground_truth_list).intersection(set(top_k_reviewer_id_list))) > 0:
                hit_total += 1

    # Calculate NDCG and hit rate
    ndcg = NDCGatK_r(label_all, np.array(pred_all), top_k)
    hit_rate = hit_total / paper_number if paper_number > 0 else 0  # Ensure no division by zero

    return recall_total / paper_number, precision_total / paper_number, ndcg, hit_rate
...
if __name__ == '__main__':
    paper_filepath = r'/home/aimingshu/project/Reviewer-Rec-main/reviewer_rec_TFIDF/get_paper_embedding/data_8k/paper_detail.json'
    paper_reviewer_training_part_filepath = r'/home/aimingshu/project/Reviewer-Rec-main/reviewer_rec_TFIDF/get_paper_embedding/data_8k/train.txt'
    paper_reviewer_full_filepath = r'/home/aimingshu/project/Reviewer-Rec-main/reviewer_rec_TFIDF/get_paper_embedding/data_8k/full.txt'

    submission_history = r'/home/aimingshu/project/Reviewer-Rec-main/reviewer_rec_TFIDF/get_paper_embedding/data_8k/submission_list.txt'
    ...
    reviewer_record_dict = get_reviewer_history(paper_reviewer_training_part_filepath)
    print('data loaded')

    top_k = 20
    # paper_embedding_dict=get_paper_embedding(id_text_dict,paper_mapping_dict)
    # training_reviewer_embedding_dict=get_reviewer_embedding(paper_mapping_dict,reviewer_record_dict)

    training_reviewer_embedding_dict = read_json_file(
        r'/home/aimingshu/project/Reviewer-Rec-main/reviewer_rec_TFIDF/get_paper_embedding/embedding_data/training_reviewer_embedding.json')
    paper_embedding_dict = read_json_file(
        r'/home/aimingshu/project/Reviewer-Rec-main/reviewer_rec_TFIDF/get_paper_embedding/embedding_data/paper_embedding.json')
    ...
    recall_total, precision_total, ndcg_total,hit_total = abc(training_reviewer_embedding_dict, paper_embedding_dict, top_k,
                                                    review_history_dict, review_history_training_dict)
    print(recall_total, precision_total, ndcg_total,hit_total)
