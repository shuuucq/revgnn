...
test_inner_file = 'data_4k/test.txt'
train_inner_file = 'data_4k/train.txt'

paper_info_file = 'data_4k/new_paper_info.json'
reviewer_info_file = 'data_4k/new_reviewer_info.json'
...
for pap_id in similarities.keys():
    ...
    hr20.append(calculate_hr_at_k(pap_rev_ids_pred, test_inner_pap_rev_dict[pap_id], k=20))
    precision20.append(calculate_precision_at_k(pap_rev_ids_pred, test_inner_pap_rev_dict[pap_id], k=20))


print("NDCG@20: {:.4f}, Recall@20: {:.4f},Hr@20: {:.4f},Precision@20:{:.4f}".format(np.mean(ndcg20), np.mean(recall20),np.mean(hr20),np.mean(precision20)))